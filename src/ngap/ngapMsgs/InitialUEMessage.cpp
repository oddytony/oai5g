...
void InitialUEMessageMsg::setMessageType() {
  ...
  if (InitialUEMessageMessageTypeIE.getProcedureCode()
    ...
    cout
        << "[warning] This information doesn't refer to InitialUEMessage Message!!!"
        << endl;
  }
}
...
//------------------------------------------------------------------------------
void InitialUEMessageMsg::setUserLocationInfoNR(struct NrCgi_s cig,
                                                struct Tai_s tai) {
...
int InitialUEMessageMsg::encode2buffer(uint8_t *buf, int buf_size) {
  ...
  asn_enc_rval_t er = aper_encode_to_buffer(&asn_DEF_Ngap_NGAP_PDU, NULL,
                                            initialUEMessagePdu, buf, buf_size);
      ...
      case Ngap_ProtocolIE_ID_id_RAN_UE_NGAP_ID: {
        if (initialUEMessageIEs->protocolIEs.list.array[i]->criticality
          ...
          if (!ranUeNgapId->decodefromRAN_UE_NGAP_ID(
              initialUEMessageIEs->protocolIEs.list.array[i]->value.choice
                  .RAN_UE_NGAP_ID)) {
