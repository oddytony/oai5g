...
namespace ngap{
	...
	{
		if(!pduSessionResourceSetupRequestPdu) pduSessionResourceSetupRequestPdu = (Ngap_NGAP_PDU_t*)calloc(1, sizeof(Ngap_NGAP_PDU_t));
	...
	bool PduSessionResourceSetupRequestMsg::decodefrompdu(Ngap_NGAP_PDU_t *ngap_msg_pdu)
	{
		...
		{
			if(pduSessionResourceSetupRequestPdu->choice.initiatingMessage && pduSessionResourceSetupRequestPdu->choice.initiatingMessage->procedureCode == Ngap_ProcedureCode_id_PDUSessionResourceSetup && pduSessionResourceSetupRequestPdu->choice.initiatingMessage->criticality == Ngap_Criticality_reject && pduSessionResourceSetupRequestPdu->choice.initiatingMessage->value.present == Ngap_InitiatingMessage__value_PR_PDUSessionResourceSetupRequest)
			{
				pduSessionResourceSetupRequestIEs = &pduSessionResourceSetupRequestPdu->choice.initiatingMessage->value.choice.PDUSessionResourceSetupRequest;
			}
			else
