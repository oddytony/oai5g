...
void PduSessionResourceReleaseCommandMsg::setMessageType() {
  if (!pduSessionResourceReleaseCommandPdu)
    pduSessionResourceReleaseCommandPdu = (Ngap_NGAP_PDU_t*) calloc(
  ...
  if (initialMessageTypeIE.getProcedureCode()
    ...
    pduSessionResourceReleaseCommandIEs = &(pduSessionResourceReleaseCommandPdu
        ->choice.initiatingMessage->value.choice
        .PDUSessionResourceReleaseCommand);
  } else {
    Logger::nas_mm().warn(
        "This information doesn't refer to PDUSessionResourceReleaseCommand Message");
  }
}
...
void PduSessionResourceReleaseCommandMsg::setAmfUeNgapId(unsigned long id) {
  ...
  ret = ASN_SEQUENCE_ADD(&pduSessionResourceReleaseCommandIEs->protocolIEs.list,
                         ie);
  if (ret != 0)
    Logger::nas_mm().warn("Encode AMF_UE_NGAP_ID IE error");
}
...
//------------------------------------------------------------------------------
void PduSessionResourceReleaseCommandMsg::setNasPdu(uint8_t *nas,
                                                    size_t sizeofnas) {
  if (!nasPdu)
    nasPdu = new NAS_PDU();
...
//------------------------------------------------------------------------------
int PduSessionResourceReleaseCommandMsg::encode2buffer(uint8_t *buf,
                                                       int buf_size) {
...
//------------------------------------------------------------------------------
void PduSessionResourceReleaseCommandMsg::encode2buffer_new(char *buf,
                                                            int &encoded_size) {
...
bool PduSessionResourceReleaseCommandMsg::decodefrompdu(
  ...
  for (int i = 0;
    ...
    switch (pduSessionResourceReleaseCommandIEs->protocolIEs.list.array[i]->id) {
...
//------------------------------------------------------------------------------
bool PduSessionResourceReleaseCommandMsg::getNasPdu(uint8_t *&nas,
                                                    size_t &sizeofnas) {
  if (!nasPdu)
    return false;
