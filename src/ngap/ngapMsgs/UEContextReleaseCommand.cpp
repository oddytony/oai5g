...
void UEContextReleaseCommandMsg::setMessageType() {
  if (!pdu)
    pdu = (Ngap_NGAP_PDU_t*) calloc(1, sizeof(Ngap_NGAP_PDU_t));
  MessageType msgType;
...
void UEContextReleaseCommandMsg::setAmfUeNgapId(unsigned long id) {
  ...
  if (ret != 0)
    cout << "encode UE_NGAP_IDs IE error" << endl;
}

//------------------------------------------------------------------------------
void UEContextReleaseCommandMsg::setUeNgapIdPair(unsigned long amfId,
                                                 uint32_t ranId) {
  if (!amfUeNgapId)
    amfUeNgapId = new AMF_UE_NGAP_ID();
