...
void InitialContextSetupRequestMsg::setMessageType() {
  if (!initialContextSetupRequestPdu)
    initialContextSetupRequestPdu = (Ngap_NGAP_PDU_t*) calloc(1, sizeof(Ngap_NGAP_PDU_t));
...
bool InitialContextSetupRequestMsg::decodefrompdu(Ngap_NGAP_PDU_t *ngap_msg_pdu) {
  ...
  if (initialContextSetupRequestPdu->present == Ngap_NGAP_PDU_PR_initiatingMessage) {
    if (initialContextSetupRequestPdu->choice.initiatingMessage && initialContextSetupRequestPdu->choice.initiatingMessage->procedureCode == Ngap_ProcedureCode_id_InitialContextSetup && initialContextSetupRequestPdu->choice.initiatingMessage->criticality == Ngap_Criticality_reject
        && initialContextSetupRequestPdu->choice.initiatingMessage->value.present == Ngap_InitiatingMessage__value_PR_InitialContextSetupRequest) {
      initialContextSetupRequestIEs = &initialContextSetupRequestPdu->choice.initiatingMessage->value.choice.InitialContextSetupRequest;
    } else {
...
bool InitialContextSetupRequestMsg::getUESecurityCapability(uint16_t &NR_EncryptionAlgs, uint16_t &NR_IntegrityProtectionAlgs, uint16_t &E_UTRA_EncryptionAlgs, uint16_t &E_UTRA_IntegrityProtectionAlgs) {
  if (!uESecurityCapabilities)
    return false;
  if (!uESecurityCapabilities->getUESecurityCapabilities(NR_EncryptionAlgs, NR_IntegrityProtectionAlgs, E_UTRA_EncryptionAlgs, E_UTRA_IntegrityProtectionAlgs))
    return false;
...
bool InitialContextSetupRequestMsg::getSecurityKey(uint8_t *&key) {
  if (!securityKey)
    return false;
  if (!securityKey->getSecurityKey(key))
    return false;
