...
void InitialContextSetupResponseMsg::setMessageType() {
  if (!initialContextSetupResponsePdu)
    initialContextSetupResponsePdu = (Ngap_NGAP_PDU_t*) calloc(
  ...
  if (initialContextSetupResponsePduTypeIE.getProcedureCode()
    ...
    initialContextSetupResponseIEs = &(initialContextSetupResponsePdu->choice
        .successfulOutcome->value.choice.InitialContextSetupResponse);
  } else {
    std::cout
        << "[Warning] This information doesn't refer to InitialContextSetupResponse message!"
        << std::endl;
  }
}
...
int InitialContextSetupResponseMsg::encode2buffer(uint8_t *buf, int buf_size) {
  ...
  asn_enc_rval_t er = aper_encode_to_buffer(&asn_DEF_Ngap_NGAP_PDU, NULL,
                                            initialContextSetupResponsePdu, buf,
                                            buf_size);
    ...
    switch (initialContextSetupResponseIEs->protocolIEs.list.array[i]->id) {
      case Ngap_ProtocolIE_ID_id_AMF_UE_NGAP_ID: {
        if (initialContextSetupResponseIEs->protocolIEs.list.array[i]
            ...
            && initialContextSetupResponseIEs->protocolIEs.list.array[i]->value
                .present
                == Ngap_InitialContextSetupResponseIEs__value_PR_AMF_UE_NGAP_ID) {
          amfUeNgapId = new AMF_UE_NGAP_ID();
      ...
      case Ngap_ProtocolIE_ID_id_PDUSessionResourceFailedToSetupListCxtRes: {
        if (initialContextSetupResponseIEs->protocolIEs.list.array[i]
          ...
          if (!pduSessionResourceFailedToSetupResponseList
              ...
              &initialContextSetupResponseIEs->protocolIEs.list.array[i]->value
                  .choice.PDUSessionResourceFailedToSetupListCxtRes)) {
            std::cout
                << "Decoded NGAP PDUSessionResourceFailedToSetupListCxtRes IE error"
                << std::endl;
            return false;
          }
        } else {
