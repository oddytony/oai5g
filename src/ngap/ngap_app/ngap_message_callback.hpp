...
int ngap_amf_handle_initial_context_setup_response(
  ...
  if (!initCtxResp->getPduSessionResourceSetupResponseList(list)) {
    Logger::ngap().error("Decode PduSessionResourceSetupResponseList IE error "
                         "or this IE is not available");
    return 0;
  }
...
int ngap_amf_handle_pdu_session_resource_setup_response(
  ...
  if (!pduresp->getPduSessionResourceSetupResponseList(list)) {
    Logger::ngap().error("Decoding PduSessionResourceSetupResponseMsg "
...
//------------------------------------------------------------------------------
int handover_preparation_handoverRequire(const sctp_assoc_id_t assoc_id,
                                         const sctp_stream_id_t stream,
                                         struct Ngap_NGAP_PDU *message_p) {
...
//------------------------------------------------------------------------------
int handoverResourceAllocation_handoverrequestAck(
    const sctp_assoc_id_t assoc_id, const sctp_stream_id_t stream,
    struct Ngap_NGAP_PDU *message_p) {
  Logger::ngap().debug(
...
//------------------------------------------------------------------------------
int handoverNotification_handovernotify(const sctp_assoc_id_t assoc_id,
                                        const sctp_stream_id_t stream,
                                        struct Ngap_NGAP_PDU *message_p) {
...
ngap_message_decoded_callback messages_callback[][3] = {
    ...
    {0, handoverNotification_handovernotify, 0},  /*11 HandoverNotification*/
    {handover_preparation_handoverRequire, 0, 0}, /*12 HandoverPreparation*/
    {0, handoverResourceAllocation_handoverrequestAck,
     0}, /*13 HandoverResourceAllocation*/
