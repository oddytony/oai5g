...
bool GBR_QosInformation::encode2GBR_QosInformation(
  ...
  gBR_QosInformation->maximumFlowBitRateDL.buf = (uint8_t*) calloc(
      1, gBR_QosInformation->maximumFlowBitRateDL.size);
  if (!gBR_QosInformation->maximumFlowBitRateDL.buf)
    return false;

  for (int i = 0; i < gBR_QosInformation->maximumFlowBitRateDL.size; i++) {
    gBR_QosInformation->maximumFlowBitRateDL.buf[i] = (maximumFlowBitRateDL
        & (0xff0000000000 >> i * 8))
        >> ((gBR_QosInformation->maximumFlowBitRateDL.size - i - 1) * 8);
  }

  gBR_QosInformation->maximumFlowBitRateUL.size = 6;
  gBR_QosInformation->maximumFlowBitRateUL.buf = (uint8_t*) calloc(
      1, gBR_QosInformation->maximumFlowBitRateUL.size);
  if (!gBR_QosInformation->maximumFlowBitRateUL.buf)
    return false;

  for (int i = 0; i < gBR_QosInformation->maximumFlowBitRateUL.size; i++) {
    gBR_QosInformation->maximumFlowBitRateUL.buf[i] = (maximumFlowBitRateUL
        & (0xff0000000000 >> i * 8))
        >> ((gBR_QosInformation->maximumFlowBitRateUL.size - i - 1) * 8);
  }

  gBR_QosInformation->guaranteedFlowBitRateDL.size = 6;
  gBR_QosInformation->guaranteedFlowBitRateDL.buf = (uint8_t*) calloc(
      1, gBR_QosInformation->guaranteedFlowBitRateDL.size);
  ...
  gBR_QosInformation->guaranteedFlowBitRateUL.buf = (uint8_t*) calloc(
      1, gBR_QosInformation->guaranteedFlowBitRateUL.size);
  ...
  if (maximumPacketLossRateDL) {
    Ngap_PacketLossRate_t *mplrd = (Ngap_PacketLossRate_t*) calloc(
        1, sizeof(Ngap_PacketLossRate_t));
    if (!mplrd)
      return false;
  ...
  if (maximumPacketLossRateUL) {
    Ngap_PacketLossRate_t *mplru = (Ngap_PacketLossRate_t*) calloc(
        1, sizeof(Ngap_PacketLossRate_t));
    if (!mplru)
      return false;
