...
//------------------------------------------------------------------------------
UES_Usage_Setting::UES_Usage_Setting(const uint8_t iei, bool ues_usage_setting) {
  _iei = iei;
...
int UES_Usage_Setting::encode2buffer(uint8_t *buf, int len) {
  ...
  if (_iei) {
    *(buf + encoded_size) = _iei;
    encoded_size++;
    Logger::nas_mm().debug("encoded UES_Usage_Setting len(%d)", *(buf + encoded_size - 1));
    *(buf + encoded_size) = 1;
    encoded_size++;
...
//	length = *(buf + decoded_size); 
  ...
  decoded_size++;
  Logger::nas_mm().debug("decoded UES_Usage_Setting _ues_usage_setting(0x%x),iei(0x%x)", _ues_usage_setting, _iei);
