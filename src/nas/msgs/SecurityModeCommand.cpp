...
//------------------------------------------------------------------------------
void SecurityModeCommand::setNAS_Security_Algorithms(uint8_t ciphering,
                                                     uint8_t integrity) {
  ie_selected_nas_security_algorithms = new NAS_Security_Algorithms(ciphering,
                                                                    integrity);
}
...
//------------------------------------------------------------------------------
void SecurityModeCommand::setUE_Security_Capability(uint8_t g_EASel,
                                                    uint8_t g_IASel) {
...
//------------------------------------------------------------------------------
void SecurityModeCommand::setEPS_NAS_Security_Algorithms(uint8_t ciphering,
                                                         uint8_t integrity) {
  ie_eps_nas_security_algorithms = new EPS_NAS_Security_Algorithms(0x57,
                                                                   ciphering,
                                                                   integrity);
}

//------------------------------------------------------------------------------
void SecurityModeCommand::setAdditional_5G_Security_Information(bool rinmr,
                                                                bool hdp) {
...
//------------------------------------------------------------------------------
void SecurityModeCommand::setS1_UE_Security_Capability(uint8_t g_EEASel,
                                                       uint8_t g_EIASel) {
  ie_s1_ue_security_capability = new S1_UE_Security_Capability(0x19, g_EEASel,
                                                               g_EIASel);
}
...
int SecurityModeCommand::encode2buffer(uint8_t *buf, int len) {
  ...
  if (!ie_imeisv_request) {
    ...
    if (int size = ie_imeisv_request->encode2buffer(buf + encoded_size,
                                                    len - encoded_size)) {
      encoded_size += size;
    } else {
  ...
  Logger::nas_mm().debug("Encoded SecurityModeCommand message len (%d)",
                         encoded_size);
  return encoded_size;
}

//------------------------------------------------------------------------------
int SecurityModeCommand::decodefrombuffer(NasMmPlainHeader *header,
                                          uint8_t *buf, int len) {
  ...
  while ((octet != 0x0)) {
    switch ((octet & 0xf0) >> 4) {
      case 0xE: {
        ...
        decoded_size += ie_imeisv_request->decodefrombuffer(buf + decoded_size,
                                                            len - decoded_size,
                                                            true);
  ...
  Logger::nas_mm().debug("Decoded SecurityModeCommand message len (%d)",
                         decoded_size);
