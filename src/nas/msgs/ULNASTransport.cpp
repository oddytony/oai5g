...
void ULNASTransport::setRelease_Assistance_Indication(uint8_t value) {
  ie_release_assistance_indication = new Release_Assistance_Indication(0x0F,
                                                                       value);
}
...
int ULNASTransport::encode2buffer(uint8_t *buf, int len) {
  ...
  if (!ie_request_type) {
    ...
    if (int size = ie_request_type->encode2buffer(buf + encoded_size,
                                                  len - encoded_size)) {
      encoded_size += size;
    } else {
  ...
  if (!ie_s_nssai) {
    ...
    if (int size = ie_s_nssai->encode2buffer(buf + encoded_size,
                                             len - encoded_size)) {
      encoded_size += size;
    } else {
  ...
  if (!ie_dnn) {
    ...
    if (int size = ie_dnn->encode2buffer(buf + encoded_size,
                                         len - encoded_size)) {
      encoded_size += size;
    } else {
  ...
  Logger::nas_mm().debug("encoded ULNASTransport message len(%d)",
                         encoded_size);
  return 1;
}

//------------------------------------------------------------------------------
int ULNASTransport::decodefrombuffer(NasMmPlainHeader *header, uint8_t *buf,
                                     int len) {
  ...
  while ((octet != 0x0)) {
    switch ((octet & 0xf0) >> 4) {
      case 0x8: {
        ...
        decoded_size += ie_request_type->decodefrombuffer(buf + decoded_size,
                                                          len - decoded_size,
                                                          true);
    ...
    switch (octet) {
      ...
      case 0x22: {
        ...
        decoded_size += ie_s_nssai->decodefrombuffer(buf + decoded_size,
      ...
      case 0x25: {
        ...
        ie_dnn = new DNN();
        decoded_size += ie_dnn->decodefrombuffer(buf + decoded_size,
                                                 len - decoded_size, true);
        octet = *(buf + decoded_size);
        Logger::nas_mm().debug("Next IEI (0x%x)", octet);
      }
  ...
  Logger::nas_mm().debug("decoded ULNASTransport message len(%d)",
                         decoded_size);
