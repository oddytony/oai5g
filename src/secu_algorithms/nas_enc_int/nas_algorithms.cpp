...
int nas_algorithms::nas_stream_encrypt_nea1(
  ...
  IV[2] = ((((uint32_t) stream_cipher->bearer) << 3)
      | ((((uint32_t) stream_cipher->direction) & 0x1) << 2)) << 24;
  IV[1] = IV[3];
  IV[0] = IV[2];
  ...
  if (zero_bit > 0) {
    ...
    stream_cipher->message[ceil_index - 1] = stream_cipher->message[ceil_index
        - 1] & (uint8_t)(0xFF << (8 - zero_bit));
  }
  free(KS);
  KS = NULL;
...
int nas_algorithms::nas_stream_encrypt_nia1(
  ...
  for (int i = 0; i < D - 2; i++) {
    V = EVAL
        ^ ((uint64_t) hton_int32 (message[2 * i]) << 32
...
int nas_algorithms::nas_stream_encrypt_nea2(
  ...
  if (zero_bit > 0)
    data[byte_length - 1] = data[byte_length - 1]
...
int nas_algorithms::nas_stream_encrypt_nia2(
  ...
  Logger::amf_n1().trace("Byte length: %u, Zero bits: %u:", m_length + 8,
                         zero_bit);
