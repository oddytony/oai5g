...
amf_app::amf_app(const amf_config &amf_cfg) {
  ...
  try {
    ...
    amf_n2_inst = new amf_n2(std::string(inet_ntoa(amf_cfg.n2.addr4)),
                             amf_cfg.n2.port);
  ...
  timer_id_t tid = itti_inst->timer_setup(amf_cfg.statistics_interval, 0,
                                          TASK_AMF_APP,
...
//------------------------------------------------------------------------------
void amf_app::set_amf_ue_ngap_id_2_ue_context(const long &amf_ue_ngap_id,
...
//------------------------------------------------------------------------------
void amf_app::set_ran_amf_id_2_ue_context(const string &ue_context_key,
                                          std::shared_ptr<ue_context> uc) {
  std::shared_lock lock(m_ue_ctx_key);
...
//4. send nas-pdu to task_amf_n1
  ...
  if (uc.get() == nullptr) {
    Logger::amf_app().error("Failed to create ue_context with ran_amf_id %s",
                            ue_context_key.c_str());
  } else {
    uc.get()->cgi = itti_msg.cgi;
    uc.get()->tai = itti_msg.tai;
    if (itti_msg.rrc_cause != -1)
      uc.get()->rrc_estb_cause = (e_Ngap_RRCEstablishmentCause) itti_msg
          .rrc_cause;
    ...
    itti_uplink_nas_data_ind *itti_n1_msg = new itti_uplink_nas_data_ind(
        TASK_AMF_APP, TASK_AMF_N1);
...
bool amf_app::generate_5g_guti(uint32_t ranid, long amfid, string &mcc,
  ...
  string ue_context_key = "app_ue_ranid_" + to_string(ranid) + ":amfid_"
      + to_string(amfid);
  if (!is_ran_amf_id_2_ue_context(ue_context_key)) {
    Logger::amf_app().error("No UE context for ran_amf_id %s, exit",
                            ue_context_key.c_str());
    return false;
  }
  std::shared_ptr<ue_context> uc;
