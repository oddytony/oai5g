...
class sctp_application {
 public:
  virtual void handle_receive(bstring payload, sctp_assoc_id_t assoc_id,
                              sctp_stream_id_t stream,
                              sctp_stream_id_t instreams,
                              sctp_stream_id_t outstreams) = 0;
  virtual void handle_sctp_new_association(sctp_assoc_id_t assoc_id,
                                           sctp_stream_id_t instreams,
                                           sctp_stream_id_t outstreams) = 0;
...
class sctp_server {
 public:
  ...
  int sctp_send_msg(sctp_assoc_id_t sctp_assoc_id, sctp_stream_id_t stream,
                    bstring *payload);
 private:
  static void* sctp_receiver_thread(void *arg);
  int getSocket();
  int sctp_read_from_socket(int sd, uint32_t m_ppid);
  int handle_assoc_change(int sd, uint32_t ppid,
